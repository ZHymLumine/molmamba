...
import os
import itertools
from caduceus import CaduceusConfig, CaduceusForMaskedLM, CaduceusForSequenceClassification
...
class MixedTextMoleculeDataset(IterableDataset):
    def __init__(self, dataset_text, dataset_molecule):
        self.dataset_text = dataset_text
        self.dataset_molecule = dataset_molecule
...
class MoleculeDataset(IterableDataset):
    def __init__(self, dataset_molecule):
        self.dataset_molecule = dataset_molecule
...
def get_model(args, config, tokenizer, logger):
    ...
    if args.mode == "pt":
        ...
        if args.model.checkpoint_path:
            ...
            # model.resize_token_embeddings(len(tokenizer))
            # model.load_state_dict(torch.load(args.model.checkpoint_path), strict=False)
            torch.cuda.empty_cache()
            logger.log_message(f"Loaded model from {args.model.checkpoint_path}")
        elif args.model.random_init:
            model = CaduceusForSequenceClassification(
...
def get_caduceus_config(args):
    config_dict = OmegaConf.to_container(args.model, resolve=True)
    caduceus_config = CaduceusConfig(**config_dict)
    return caduceus_config
...
def load_dataset_splits(args):
    ...
    return dataset_splits_text, dataset_splits_molecule
...
def get_dataloaders(tokenizer, config, args):
    if args.mode == 'pt':
        dataset_splits_text, dataset_splits_molecule = load_dataset_splits(args)
        dataset_text = process_dataset(dataset_splits=dataset_splits_text, args=args, tokenizer=tokenizer)  # tokenize dataset
        ...
        is_iterable = isinstance(dataset_text['train'], IterableDataset) & isinstance(dataset_molecule['train'], IterableDataset)
        ...
        for split in ['train', 'test']:
            ...
            # mixed_dataset_split = MixedTextMoleculeDataset(dataset_text[split], dataset_molecule[split])
            mixed_dataset_split = MoleculeDataset(dataset_molecule[split])

            # for i, item in enumerate(mixed_dataset_split):
            #     print(item)
            #     if i >= 5:  # 因为索引是从0开始的，所以需要n-1
            #         break
    ...
    if args.mode == 'pt': return dataloaders['train'], dataloaders['test']

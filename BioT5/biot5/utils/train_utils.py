...
def maybe_save_checkpoint(accelerator, args, model):
    if (
...
def maybe_grad_clip_and_grad_calc(accelerator, model, args):
    if args.logging.grad_l2:
        grad_l2 = (
            sum(p.grad.detach().data.norm(2).item() ** 2 for p in model.parameters() if p.requires_grad) ** 0.5
        )
    else:
        grad_l2 = None
...
def predict_molnet(model, dataloader, logger, args, tokenizer, accelerator, prefix='test') :
    args.last_log = time.time()

    if args.test_task == 'molnet':
        metric = evaluate.load(os.path.join(__file__.split('biot5/utils')[0], 'biot5/metrics/molnet_metrics'))
    else:
        ...
        for batch_id, batch in enumerate(train_dataloader, start=1):
            ...
            if batch_id % args.optim.grad_acc == 0:
                ...
                if accelerator.is_main_process:
                    ...
                    maybe_save_checkpoint(accelerator, args, model)

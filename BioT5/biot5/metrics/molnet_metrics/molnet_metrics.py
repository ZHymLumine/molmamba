...
_DESCRIPTION = """\
Classification metrics for MOLNET.
"""
...
class DTI_classification(evaluate.Metric):
    ...
    def _compute(self, predictions, references, tsv_path="tmp.tsv"):
        ...
        # print(f"y_pred type : {type(y_pred)},  {y_pred}")
        # print(f"y_true type : {type(y_true)},  {y_true}")
        # print(f"tpr type : {type(tpr)},  {tpr}")
        # print(f"fpr type : {type(fpr)},  {fpr}")
        # print(f"precision type : {type(precision)},  {precision}")
        # print(f"f1 type : {type(f1)},  {f1}")
